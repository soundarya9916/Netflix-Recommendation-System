...
NetflixDF <-  read.csv(file = "netflix_titles.csv", na.strings = c("NA", ""), stringsAsFactors=F)

##Fetch the weighted average votes/ratings of the movies from imdb for netflix

RatingsIMDB=fread("IMDb ratings.csv", select = c("weighted_average_vote"))
TitlesIMDB=fread("IMDb movies.csv", select = c('title','year','genre','reviews_from_users'))
...
movie_linear = lm(weighted_average_vote ~ rating + genre  + reviews_from_users + country + listed_in + duration,data = training,drop.unused.levels = TRUE)
summary(movie_linear)
#lin_pred=predict(movie_linear, newdata=new_val)
...
#MSE of GBM
predicted_boost=predict(movie.boost,newdata=validation,n.trees=5000)
MSEgbm<-mean((validation$weighted_average_vote - predicted_boost)^2)
MSEgbm
...
#decison Tree
library(rpart)
fit<-rpart(weighted_average_vote ~ rating + genre  + reviews_from_users + country + listed_in + duration,method = "anova",data = training)
# Output to be present as PNG file
png(file = "decTree2GFG.png", width = 1000,
    height = 1000)

# Plot
plot(fit, uniform = TRUE,
     main = "Netflix Recommendation ")
...
#mse of decison tree
MSE<-mean((validation$weighted_average_vote - predictions)^2)
MSE
